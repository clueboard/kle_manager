...
if ! [ -f setup.py -a -f web.py ]; then
    echo 'You must run this in the top level of kle_manager!'
    exit 1
elif [ -d .kle_manager-$PYTHON_VERSION -a "$1" != "-f" ]; then
...
if [ -d .kle_manager-$PYTHON_VERSION ]; then
    while true; do
        ...
        case $answer in
            yes|YES)
                rm -rf .kle_manager-$PYTHON_VERSION activate-$PYTHON_VERSION
                break
                ;;
            no|NO)
...
virtualenv -p python$PYTHON_VERSION .kle_manager-$PYTHON_VERSION || exit
ln -s .kle_manager-$PYTHON_VERSION/bin/activate activate-$PYTHON_VERSION || exit
...
# If they also have kle2svg checked out use that locally
if [ -d ../kle2svg ]; then
    pip uninstall kle2svg
    pip install -e ../kle2svg || exit
fi
